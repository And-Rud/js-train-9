...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    let result = arr.find((item) => item > num);
    return result > num ? result : `Числа яке задовільняе умову не знайдено`;
  } else {
    return undefined;
  }
}
...
function findIndexByProperty(arr, name) {
  if (Array.isArray(arr)) {
    let ind = arr.findIndex((item) => item.name === name);
    return ind === -1 ? -1 : ind;
  } else {
    return -1;
  }
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr)) {
    let res = arr.findLast((item) => item % 2 === 0);
    return res ? res : `Парних чисел не знайдено`;
  } else {
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    ...
    let sum = res.reduceRight((acc, item) => acc + item, 0);
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr) && typeof n === "number") {
    const res = arr.flatMap((item) => item.toString().repeat(n)).join("");
    const res2 = Array.from(res);
    return res2.flatMap((item) => Number(item));
  } else {
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    return arr.reduce((max, n) => (max.length > n.length ? max : n), 0);
  } else {
    return ``;
  }
}
